---
title: The Problem
description: Why prose-only agent instructions decay, and what Piton is trying to solve.
sidebar:
  order: 4
---
...
But, I would argue that for a sufficiently complex system, something more
...
relevant as more people work on an agentic codebase; the prompts that led to an
outcome live within the session, and at best are a faint memory in the one mind
of the one developer. Despite how it may seem, neither agents nor humans can read
minds.

Many of the agentic platforms provide mechanisms like `AGENTS.md` and `CLAUDE.md`
that provide a level of instruction to the agent. Further still, more granular
skills and agents are written in Markdown and committed to the repository. Now
...
is likely to inadvertently describe subsystems and instructions with phrasing that
